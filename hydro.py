...
#############################
# # Adjust parameters according to your actual data
# days = 10  # Adjust according to your simulation duration
# for day in range(days):
#     avg_wt_over_time_result = hydrology(
#     solve_mode='transient',
#     nx=100,  # Number of grid cells in the x-direction
#     ny=100,  # Number of grid cells in the y-direction
#     dx=10,   # Grid cell size in the x-direction
#     dy=10,   # Grid cell size in the y-direction
#     days=10, # Number of simulation days
#     ele=np.zeros((100, 100)),           # Initial elevation array
#     phi_initial=np.zeros((100, 100)),   # Initial water table depth array
#     catchment_mask=np.ones((100, 100)), # Catchment mask array
#     wt_canal_arr=np.zeros((100, 100)),  # Canal water level array
#     boundary_arr=np.zeros((100, 100)),  # Boundary condition array
#     peat_type_mask=np.zeros((100, 100)),# Peat type mask array
#     httd={},                            # Dictionary containing interpolation functions for T and storage coeff
#     tra_to_cut=np.zeros((100, 100)),    # T to be cut
#     sto_to_cut=np.zeros((100, 100)),    # Storage to be cut
#     diri_bc=0.0,                        # Dirichlet boundary condition
#     neumann_bc=None,                    # Neumann boundary condition
#     plotOpt=True,                      # Whether to enable plotting or not
#     remove_ponding_water=True,          # Whether to remove ponding water
#     P=np.zeros(10),                     # Precipitation data for 10 days
#     ET=np.zeros(10),                    # Evapotranspiration data for 10 days
#     dt=1.0                              # Time step
#     )
    
#     avg_wt_over_time_list.append(avg_wt_over_time_result)
#     # Append other results to their respective lists

# # Create a DataFrame for avg_wt_over_time
# avg_wt_over_time_df = pd.DataFrame(avg_wt_over_time_list, columns=['avg_wt_over_time'])

# # Export the DataFrame to an Excel file
# avg_wt_over_time_df.to_excel('avg_wt_over_time_results.xlsx', index=False)

#############################
# Convert lists to numpy arrays for consistency
# avg_wt_array = np.array(avg_wt)
# cumulative_vdp_array = np.array(cumulative_vdp)
# d_array = np.array(d)

# # Assuming P is a numpy array
# P_array = P if isinstance(P, np.ndarray) else np.array([P])

# # Create a dictionary containing the variables
# data = {
#     'avg_wt': avg_wt_array,
#     'cumulative_vdp': cumulative_vdp_array,
#     'd': d_array,
#     'P': P_array
# }
...
# df.to_excel('hydro_data.xlsx', index=False)

# import rasterio
# from rasterio.transform import from_origin

# def export_to_geotiff(raster, filename, transform, crs):
...
# # Assuming these are the rasters you want to export
# # Assuming these rasters are available after computation
# # Replace these with the actual raster data arrays
# raster1_after_computation = ((hydro_utils.peat_map_h_to_tra(soil_type_mask=peat_type_mask, gwt=(phi.value - ele), h_to_tra_and_C_dict=httd) - tra_to_cut)*cmask.value *drmask_not.value ).reshape(ny,nx)
# raster2_after_computation = (ele.reshape(ny,nx) - wt_canal_arr) * dr * catchment_mask
# raster3_after_computation = ele.reshape(ny,nx)
# raster4_after_computation = (ele-phi.value).reshape(ny,nx)

# rasters = [raster1_after_computation, raster2_after_computation, raster3_after_computation, raster4_after_computation]

# # Assuming these are the transformation and CRS information for the rasters
# # Replace these with the appropriate values for your data
# transform = from_origin(x_coordinate_of_upper_left_corner, y_coordinate_of_upper_left_corner, pixel_width, pixel_height)
# crs = "EPSG:32748"  # UTM Zone 48S coordinate reference system

# # Export each raster to GeoTIFF
# for raster, filename in zip(rasters, output_files):
#     export_to_geotiff(raster, filename, transform, crs)
